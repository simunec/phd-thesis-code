% Test performance of gapfinder with increasing matrix size
...
for idx_n = 1:length(nn)
	...
	tic;
	[gaps, trest, trest_upper, trest_lower, ~, its_lanc(idx_n)] = gapfinder_main(A, mus, delta, theta, bound_type, d, c);
	t(idx_n) = toc;
...
table = "";
for j = 1:length(nn)
	tablerow = sprintf("%d & [%.2f, %.2f] & [%.2f, %.2f] & %d & %d & %.3f & %.3f \\\\", nn(j), gap_true{j}, gap_est{j}, eigcount_est(j), its_lanc(j), t(j), t_eig(j));

	table = sprintf("%s\n%s", table, tablerow);
end

fname = "tables/table_gapfinder_size.txt";
fid = fopen(fname, 'a');
fprintf(fid, "%s\n\n", table);
fclose(fid);
